...
case class CsrMapping() extends Area with CsrInterface {
  ...
  override def onRead(csrAddress: Int)(body: => Unit): Unit = addMappingAt(csrAddress, CsrOnRead(() => {
    body
  }))
  ...
  override def build(pipeline: VexRiscv): Unit = {
    ...
    val supervisorCsr = ifGen(supervisorGen) {
      pipeline plug new Area {
        ...
        val rescheduleLogic = supervisorGen generate new Area {
          ...
          when(execute.arbitration.isValid && execute.input(IS_SFENCE_VMA)) {
            rescheduleNext := True
          }
    ...
    pipeline plug new Area {
      ...
      val exceptionPortCtrl = exceptionPortsInfos.nonEmpty generate new Area {
        ...
        var firstStageIndexWithExceptionPort = exceptionPortsInfos.map(i => indexOf(i.stage)).min
        if (firstStageIndexWithExceptionPort < 0) firstStageIndexWithExceptionPort = 0
        println(s"firstStageIndexWithExceptionPort = ${firstStageIndexWithExceptionPort}")
        ...
        var portCnt = stages.size - 1
        for (portInfo <- sortedByStage; port = portInfo.port; stage = portInfo.stage; stageId = indexOf(portInfo.stage)) {
          if (portCnt > 0) {
            val stageIdUse = if (stageId > 0) stageId else stages.map(_.toString().split("/").last).indexOf(portInfo.stage.toString.split("/").last)
            println(s"stageIdUse = ${stageIdUse}, stage = ${stage}, stages = ${stages}")
            when(port.valid) {
              stage.arbitration.flushNext := True
              stage.arbitration.removeIt := True
              exceptionValids(stageIdUse) := True
      ...
      switch(targetPrivilege) {
        if (supervisorGen) is(1) {
          xtvec := supervisorCsr.stvec
        }
        is(3) {
          xtvec := machineCsr.mtvec
        }
      }
      ...
      execute plug new Area {
        ...
        if (ecallGen) when(arbitration.isValid && input(ENV_CTRL) === EnvCtrlEnum.ECALL) {
          ...
          switch(privilege) {
            is(0) {
              selfException.code := 8
            }
            if (supervisorGen) is(1) {
              selfException.code := 9
            }
            default {
        ...
        Component.current.afterElaboration {
          def doJobs(jobs: ArrayBuffer[Any]): Unit = {
            ...
            for (element <- jobs) element match {
              case element: CsrDuringWrite => when(writeInstruction) {
                element.doThat()
              }
              case element: CsrDuringRead => when(readInstruction) {
                element.doThat()
              }
              case element: CsrDuring => {
                element.doThat()
              }
              case _ =>
            }
          ...
          csrMapping.always.foreach {
            ...
            case element: CsrOnWrite => when(writeEnable) {
              element.doThat()
            }
            case element: CsrOnRead => when(readEnable) {
              element.doThat()
            }
          }
