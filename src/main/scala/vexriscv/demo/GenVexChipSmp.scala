...
import vexriscv.plugin.{BranchPlugin, CsrAccess, CsrPlugin, CsrPluginConfig, DBusCachedPlugin, DBusSimpleBus, DBusSimplePlugin, DYNAMIC_TARGET, DebugPlugin, DecoderSimplePlugin, DivPlugin, FullBarrelShifterPlugin, HazardSimplePlugin, IBusCachedPlugin, IBusSimpleBus, IBusSimplePlugin, IntAluPlugin, MulPlugin, PcManagerSimplePlugin, Plugin, RegFilePlugin, SimpleFormalPlugin, SrcPlugin, StaticMemoryTranslatorPlugin, YamlPlugin}
...
object VexChipSmpConfig {
  ...
  def default = VexChipSmpConfig(cpuCount = defaultCpuCount,
    configs = (0 until defaultCpuCount).map(i => VexAxiConfig.default.copy(iCacheSize = 0, dCacheSize = 0, hartId = i)))
}
...
class VexChipSmp(config: VexChipSmpConfig) extends Module {
  ...
  case class CpuPorts(iBus: Axi4ReadOnly, dBus: Axi4Shared)

  val cpus = configs.map(con => {
    import con._
    ...
    cpu.setDefinitionName(config.name + "_cpu" + con.hartId)
    CpuPorts(iBus, dBus)
  })
}
