...
object VexAxiCore {
  ...
  def main(args: Array[String]): Unit = {
    SpinalConfig().generateVerilog(new VexAxiCore(VexAxiConfig.default))
  }
}
  ...
  cpu.rework {
    ...
    for (plugin <- cpuConfig.plugins) plugin match {
      ...
      case plugin: DBusSimplePlugin => {
        ...
        dBus = master(plugin.dBus.toAxi4Shared())
          .setName("dBus")
    ...
    for (plugin <- cpuConfig.plugins) plugin match {
      ...
      case plugin: SimpleFormalPlugin => {
        ...
        rvfi = master(plugin.rvfi).setName("rvfi")
      }
      case _ =>
    }
  }
