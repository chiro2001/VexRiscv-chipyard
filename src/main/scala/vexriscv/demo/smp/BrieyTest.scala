...
case class BrieyTestConfig(axiFrequency: HertzNumber,
...
object BrieyTestConfig {

  def default = {
    val config = BrieyTestConfig(
      axiFrequency = 50 MHz,
      onChipRamSize = 4 kB,
      ...
      cpuPlugins = ArrayBuffer(
        ...
        new IBusSimplePlugin(
          ...
          cmdForkPersistence = true, cmdForkOnSecondStage = true
        ),
...
class BrieyTest(val config: BrieyTestConfig) extends Component {

  //Legacy constructor
  def this(axiFrequency: HertzNumber) {
    this(BrieyTestConfig.default.copy(axiFrequency = axiFrequency))
  }
  ...
  val axi = new ClockingArea(axiClockDomain) {
    ...
    val core = (0 until 2).map(cpuId => new Area {
    ...
    axiCrossbar.addConnections(
      ...
      core(0).iBus -> List(ram.io.axi, sdramCtrl.io.axi),
      core(0).dBus -> List(ram.io.axi, sdramCtrl.io.axi, apbBridge.io.axi),
      core(1).iBus -> List(ram.io.axi, sdramCtrl.io.axi),
      core(1).dBus -> List(ram.io.axi, sdramCtrl.io.axi, apbBridge.io.axi),
...
object BrieyTest {
  def main(args: Array[String]) {
    ...
    config.generateVerilog({
      val toplevel = new BrieyTest(BrieyTestConfig.default)
...
object BrieyTestWithMemoryInit {
  def main(args: Array[String]) {
    val config = SpinalConfig()
    config.generateVerilog({
      val toplevel = new BrieyTest(BrieyTestConfig.default)
...
object BrieyTestDe0Nano {
  def main(args: Array[String]) {
    ...
    val config = SpinalConfig()
    config.generateVerilog({
      val toplevel = new BrieyTest(BrieyTestConfig.default.copy(sdramLayout = IS42x160G.layout))
      toplevel
    })
  }
}
...
object BrieyTestSim {
  def main(args: Array[String]): Unit = {
    ...
    SimConfig.allOptimisation.compile(new BrieyTest(BrieyTestConfig.default)).doSimUntilVoid { dut =>
